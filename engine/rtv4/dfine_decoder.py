...
class SobelDepthwise(nn.Module):
    """Channel-wise Sobel gradient magnitude extractor."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        c = x.shape[1]
        kx = self.kx.view(1, 1, 3, 3).repeat(c, 1, 1, 1)
        ky = self.ky.view(1, 1, 3, 3).repeat(c, 1, 1, 1)
...
class TransformerDecoder(nn.Module):
    ...
    @staticmethod
    def _sample_reg_tokens(reg_feat_map: torch.Tensor, ref_xy: torch.Tensor) -> torch.Tensor:
        # ref_xy: [B, NQ, 2] in normalized [0, 1]
        grid = ref_xy.clamp(min=0.0, max=1.0) * 2.0 - 1.0
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layers):
            ...
            # Regression-only feature fusion: gradient tokens are injected only into bbox branch.
            bbox_in = output + output_detach
            if reg_grad_feat_map is not None and reg_grad_gate is not None:
                reg_token = self._sample_reg_tokens(reg_grad_feat_map, ref_points_detach[..., :2])
                if reg_grad_proj is not None:
                    reg_token = reg_grad_proj[i](reg_token)
                if reg_token.shape[-1] != bbox_in.shape[-1]:
                    # Keep [B, NQ, C] layout and safely align channel dim only.
                    ch_delta = bbox_in.shape[-1] - reg_token.shape[-1]
                    if ch_delta > 0:
                        reg_token = F.pad(reg_token, (0, ch_delta))
                    else:
                        reg_token = reg_token[..., :bbox_in.shape[-1]]
                bbox_in = bbox_in + reg_grad_gate * reg_token

            # Refine bounding box corners using FDR, integrating previous layer's corrections
            pred_corners = bbox_head[i](bbox_in) + pred_corners_undetach
...
class DFINETransformer(nn.Module):
    ...
    def __init__(self,
                 ...
                 layer_scale=1,
                 mlp_act='relu',
                 enable_reg_gradient_fusion=False,
                 reg_gradient_gate_init=0.05,
                 ):
        super().__init__()
        ...
        self.reg_max = reg_max
        self.enable_reg_gradient_fusion = bool(enable_reg_gradient_fusion)
      ...
      # denoising
        ...
        # High-level gradient enhancement for regression branch only.
        if self.enable_reg_gradient_fusion:
            self.reg_gradient_sobel = SobelDepthwise()
            high_in_ch = feat_channels[-1]
            self.reg_gradient_align = nn.Sequential(OrderedDict([
                ('dw_conv', nn.Conv2d(high_in_ch, high_in_ch, kernel_size=3, stride=1, padding=1, groups=high_in_ch, bias=False)),
                ('dw_bn', nn.BatchNorm2d(high_in_ch)),
                ('dw_act', get_activation(activation)),
                ('pw_conv', nn.Conv2d(high_in_ch, hidden_dim, kernel_size=1, stride=1, bias=False)),
                ('pw_bn', nn.BatchNorm2d(hidden_dim)),
                ('pw_act', get_activation(activation)),
            ]))
            gate_init = float(max(1e-4, min(1 - 1e-4, reg_gradient_gate_init)))
            self.reg_gradient_gate_logit = nn.Parameter(torch.logit(torch.tensor(gate_init, dtype=torch.float32)))

            layer_dims = [hidden_dim for _ in range(self.eval_idx + 1)] + [scaled_dim for _ in range(num_layers - self.eval_idx - 1)]
            self.reg_gradient_query_proj = nn.ModuleList([
                nn.Identity() if d == hidden_dim else nn.Linear(hidden_dim, d)
                for d in layer_dims
            ])
        else:
            self.reg_gradient_sobel = None
            self.reg_gradient_align = None
            self.reg_gradient_gate_logit = None
            self.reg_gradient_query_proj = None
    ...
    def _reset_parameters(self, feat_channels):
        ...
        if self.enable_reg_gradient_fusion and self.reg_gradient_query_proj is not None:
            for p in self.reg_gradient_query_proj:
    ...
    def forward(self, feats, targets=None):
        reg_grad_feat_map = None
        reg_grad_gate = None
        if self.enable_reg_gradient_fusion and self.reg_gradient_sobel is not None and self.reg_gradient_align is not None:
            high_feat = feats[-1]
            reg_grad_feat_map = self.reg_gradient_align(self.reg_gradient_sobel(high_feat))
            reg_grad_gate = torch.sigmoid(self.reg_gradient_gate_logit)
        ...
        out_bboxes, out_logits, out_corners, out_refs, pre_bboxes, pre_logits = self.decoder(
            ...
            attn_mask=attn_mask,
            dn_meta=dn_meta,
            reg_grad_feat_map=reg_grad_feat_map,
            reg_grad_gate=reg_grad_gate,
            reg_grad_proj=self.reg_gradient_query_proj)
