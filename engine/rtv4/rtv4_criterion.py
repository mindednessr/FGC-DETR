...
from .factor_graph import factor_graph_training_loss
...
class RTv4Criterion(nn.Module):
    ...
    def __init__(self, \
        ...
        self.factor_graph_params = factor_graph_params or {}
    ...
    def loss_distillation(self, outputs, targets, indices, num_boxes, **kwargs):
        ...
        if student_feature_map is None or teacher_feature_map is None:
            if student_feature_map is not None:
                zero_device = student_feature_map.device
            elif teacher_feature_map is not None:
                zero_device = teacher_feature_map.device
            else:
                zero_device = outputs['pred_boxes'].device
            return {'loss_distill': torch.tensor(0.0,
                                                 device=zero_device, requires_grad=True)}
    ...
    def loss_factor_graph(self, outputs, targets, indices, num_boxes, **kwargs):
        return factor_graph_training_loss(
            outputs=outputs,
            targets=targets,
            indices=indices,
            num_boxes=num_boxes,
            params=self.factor_graph_params,
        )
    ...
    def forward(self, outputs, targets, **kwargs):
        ...
        for loss_name in self.losses:
            ...
            if loss_name == 'distill':
                ...
                use_uni_set = self.use_uni_set and (loss_name in ['boxes', 'local', 'fgo'])
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    # TODO, indices and num_box are different from RT-DETRv2
                    use_uni_set = self.use_uni_set and (loss in ['boxes', 'local', 'fgo'])
