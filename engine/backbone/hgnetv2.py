...
class ShallowLaplacianEnhance(nn.Module):
    """Dual-branch 1xk / kx1 Laplacian enhancement with a shared gate.

    This module is designed for shallow feature enhancement right after the
    first downsampling stage to preserve tiny crack edges.
    """
    ...
    def __init__(
        ...
        assert eca_kernel_size % 2 == 1 and eca_kernel_size >= 3, 'eca_kernel_size must be odd and >= 3'
        self.channels = int(channels)
        self.kernel_size = int(kernel_size)

        vec = torch.zeros(self.kernel_size, dtype=torch.float32)
        c = self.kernel_size // 2
        vec[c] = -2.0
        vec[c - 1] = 1.0
        vec[c + 1] = 1.0
        self.register_buffer('lap_vec', vec)

        gate_logit = torch.logit(torch.tensor(float(max(1e-4, min(1 - 1e-4, gate_init))), dtype=torch.float32))
        if gate_learnable:
            self.gate_logit = nn.Parameter(gate_logit)
        else:
            self.register_buffer('gate_logit', gate_logit)

        self.x_refine = ECAModule(self.channels, k_size=eca_kernel_size) if use_eca_on_x else nn.Identity()

    def forward(self, x):
        if x.ndim != 4 or x.shape[1] != self.channels:
            return x

        x_identity = self.x_refine(x)
        c = x.shape[1]
        kv = self.lap_vec.view(1, 1, 1, self.kernel_size).repeat(c, 1, 1, 1)
        kh = self.lap_vec.view(1, 1, self.kernel_size, 1).repeat(c, 1, 1, 1)

        edge_v = F.conv2d(x, kv, bias=None, stride=1, padding=(0, self.kernel_size // 2), groups=c)
        edge_h = F.conv2d(x, kh, bias=None, stride=1, padding=(self.kernel_size // 2, 0), groups=c)
        edge = torch.tanh(edge_v + edge_h)
        gate = torch.sigmoid(self.gate_logit)
        return x_identity + gate * edge
...
class HGNetv2(nn.Module):
    ...
    def __init__(self,
                 ...
                 use_lab=False,
                 return_idx=[1, 2, 3],
                 enable_shallow_laplacian=False,
                 laplacian_kernel_size=3,
                 laplacian_gate_init=0.08,
                 laplacian_gate_learnable=True,
                 laplacian_use_eca_on_x=False,
                 laplacian_eca_kernel_size=3,
        ...
        self.enable_shallow_laplacian = bool(enable_shallow_laplacian)
        ...
        if self.enable_shallow_laplacian:
            self.shallow_laplacian = ShallowLaplacianEnhance(
                channels=stem_channels[2],
                kernel_size=laplacian_kernel_size,
                gate_init=laplacian_gate_init,
                gate_learnable=laplacian_gate_learnable,
                use_eca_on_x=laplacian_use_eca_on_x,
                eca_kernel_size=laplacian_eca_kernel_size,
            )
        else:
            self.shallow_laplacian = nn.Identity()
