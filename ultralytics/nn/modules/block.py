...
class HOG_LDRConv(nn.Module):
    """HOG-aware lightweight dynamic representation block.

    This minimal implementation wraps C3k2 and adds a depthwise edge gate to emphasize local structure.
    """

    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False, e: float = 0.25, patch_size: int = 8):
        """Initialize HOG_LDRConv block.

        Args:
            c1 (int): Input channels.
            c2 (int): Output channels.
            n (int): Number of internal repeats.
            shortcut (bool): Whether to use residual shortcuts in base block.
            e (float): Expansion ratio.
            patch_size (int): Patch-size hint for compatibility.
        """
        super().__init__()
        self.patch_size = patch_size
        self.base = C3k2(c1, c2, n=n, c3k=False, e=e, shortcut=shortcut)
        self.edge = nn.Conv2d(c2, c2, 3, 1, 1, groups=c2, bias=False)
        self.gate = nn.Sigmoid()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Run base extraction followed by edge-aware gating."""
        y = self.base(x)
        return y + y * self.gate(self.edge(y))
...
class HOG_C2PSA(nn.Module):
    """HOG-compatible C2PSA wrapper.

    Keeps C2PSA behavior while exposing a bins argument for YAML compatibility.
    """

    def __init__(self, c1: int, c2: int, n: int = 1, bins: int = 9, e: float = 0.5):
        """Initialize HOG_C2PSA block.

        Args:
            c1 (int): Input channels.
            c2 (int): Output channels.
            n (int): Number of PSA blocks.
            bins (int): Orientation bins hint.
            e (float): Expansion ratio.
        """
        super().__init__()
        self.bins = bins
        self.base = C2PSA(c1, c2, n=n, e=e)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass through wrapped C2PSA."""
        return self.base(x)
