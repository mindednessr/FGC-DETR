...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max: int = 16, use_inner_iou: bool = False, inner_iou_ratio: float = 0.7):
        """Initialize the BboxLoss module with regularization maximum and DFL settings."""
        super().__init__()
        self.use_inner_iou = use_inner_iou
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):  # model must be de-paralleled
        ...
        use_inner_iou = bool(getattr(h, "inner_iou", False)) or m.__class__.__name__ == "Detect_HOGLoss"
        inner_iou_ratio = float(getattr(h, "inner_iou_ratio", 0.7))
        self.bbox_loss = BboxLoss(
            m.reg_max,
            use_inner_iou=use_inner_iou,
            inner_iou_ratio=inner_iou_ratio,
        ).to(device)
